...
def parse_config(config):
    ...
    if p.log_dir != None:
        LOG.set_out_file(p.log_dir)
    LOG.info("hparams: {}".format(p))
    return p
...
def train(jsonfile, Solver, rank_size=1, rank=0):
    ...
    total_batch = trainset_builder.__len__()*p.num_epochs / p.batch_size
    ...
    if epoch > p.num_epochs:
        LOG.warning("current epoch %d exceeds than num_epochs %d" % (epoch, p.num_epochs))
        exit(0)
    while epoch < p.num_epochs:
        if rank == 0:
            LOG.info(">>>>> start training in epoch %d" % epoch)
        if epoch >= p.sorta_epoch:
            trainset_builder.batch_wise_shuffle(p.batch_size)
        solver.train(trainset_builder.as_dataset(p.batch_size, p.num_data_threads), checkpointer, pbar)

        if rank == 0:
            LOG.info(">>>>> start evaluate in epoch %d" % epoch)
