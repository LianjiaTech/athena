...
def test_e2e(jsonfile, ckpt="all",):
    ...
    for key in ckpt_list:
        ...
        for samples in tqdm.tqdm(testset_builder.as_dataset(1), total=len(testset_builder)):
            prediction = checkpointer.model.tflite_model(samples['input'], training=False) - 0.07
            prediction = tf.cast(tf.math.ceil(prediction), dtype=tf.int32)
